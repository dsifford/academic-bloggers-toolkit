        ...
        for (var i = 0, ilen = lines.length; i < ilen; i++) {
            ...
            if (key === 'type') {
                ...
                if (allowDateOverride) {
                    ...
                    if (
                        !validFieldsForType ||
                        (validFieldsForType[key] &&
                            val.match(/^[0-9]{4}(?:-[0-9]{1,2}(?:-[0-9]{1,2})*)*$/))
                    ) {
                        lines[i] = '';
                    }
                }
        ...
        if (state.tmp.group_context.tip.condition) {
            if (state.tmp.group_context.tip.condition.test) {
                ...
                if (state.tmp.group_context.tip.condition.not) {
                    state.tmp.group_context.tip.force_suppress = !state.tmp.group_context.tip
                        .force_suppress;
                }
            }
        } else {
...
CSL.Engine.prototype.retrieveItem = function(id) {
    ...
    var isLegalType =
        ['bill', 'legal_case', 'legislation', 'gazette', 'regulation'].indexOf(Item.type) > -1;
    ...
    if (Item['container-title'] && this.sys.getAbbreviation) {
        ...
        if (this.transform.abbrevs[jurisdiction]['container-title']) {
            if (this.transform.abbrevs[jurisdiction]['container-title'][Item['container-title']]) {
                Item['container-title-short'] = this.transform.abbrevs[jurisdiction][
                    'container-title'
                ][Item['container-title']];
            }
        }
...
CSL.Engine.prototype.inheritOpt = function(token, attrname, parentname, defaultValue) {
    if ('undefined' !== typeof token.strings[attrname]) {
        ...
        var parentValue = this[this.tmp.root].opt.inheritedAttributes[
        ...
        if (['bill', 'gazette', 'legislation', 'regulation', 'treaty'].indexOf(Item.type) > -1) {
            if (item.locator) {
                ...
                if (!m) {
                    if (item.label) {
                        item.locator =
                            CSL.STATUTE_SUBDIV_STRINGS_REVERSE[item.label] + ' ' + item.locator;
                    } else {
            ...
            if (Item.section) {
                if (!item.locator) {
                    ...
                    if (m) {
                        ...
                        if (
                            ['[', '(', '.', ',', ';', ':', '?'].indexOf(item.locator.slice(0, 1)) >
                            -1
                        ) {
                            space = '';
                        }
...
CSL.Output.Queue.prototype.closeLevel = function(name) {
    if (name && name !== this.current.value().levelname) {
        CSL.error(
            'Level mismatch error:  wanted ' + name + ' but found ' + this.current.value().levelname
        );
    }
                ...
                if (b && b.length) {
                    ...
                    if (state.tmp.count_offset_characters) {
                        state.tmp.offset_characters +=
                            blen + blobjr.strings.suffix.length + blobjr.strings.prefix.length;
                    }
                }
            }
...
CSL.Output.Queue.prototype.renderBlobs = function(blobs, delim, in_cite, parent) {
    ...
    for (pos = 0; pos < len; pos += 1) {
        ...
        if ('string' === typeof blob) {
            ...
            if (state.tmp.count_offset_characters) {
                state.tmp.offset_characters +=
                    addme.length + blob.strings.prefix.length + strlen + blob.strings.suffix.length;
            }
        }
    }
    return ret;
...
CSL.Output.Queue.adjust = function(punctInQuote) {
    ...
    function blobIsNumber(blob) {
        return (
            ...
            (blob.blobs && blob.blobs.length === 1 && 'number' === typeof blob.blobs[0].num)
        );
    }
...
CSL.Engine.prototype.processCitationCluster = function(
    ...
    if (flag === CSL.PREVIEW) {
        ...
        var newCitationList = citationsPre
            .concat([['' + citation.citationID, citation.properties.noteIndex]])
            .concat(citationsPost);
        var newItemIds = {};
        var newItemIdsList = [];
    ...
    for (var i = 0, ilen = citation.citationItems.length; i < ilen; i += 1) {
        ...
        if (this.opt.development_extensions.locator_label_parse) {
            if (
                item.locator &&
                ['bill', 'gazette', 'legislation', 'regulation', 'treaty'].indexOf(Item.type) ===
                    -1 &&
    ...
    if (this.opt.update_mode === CSL.POSITION) {
        for (var i = 0; i < 2; i += 1) {
            ...
            for (j = 0, jlen = citations.length; j < jlen; j += 1) {
                ...
                if (
                    j > 0 &&
                    citations[j - 1].properties.noteIndex > citations[j].properties.noteIndex
                ) {
                    citationsInNote = {};
                    first_ref = {};
                    last_ref = {};
                        ...
                        if (this.opt.xclass === 'note' && this.opt.has_disambiguate) {
                            ...
                            if ('number' === typeof oldCount) {
                                ...
                                if (oldCountCheck !== newCountCheck) {
                                    for (
                                        var l = 0,
                                            llen = this.registry.citationreg.citationsByItemId[myid]
                                                .length;
                                        l < llen;
                                        l++
                                    ) {
                    ...
                    if ('undefined' === typeof first_ref[myid]) {
                        ...
                        if (j > 0) {
                            ...
                            if (citations[j - 1].sortedItems[0].slice(-1)[0].legislation_id) {
                                oldlastid = citations[j - 1].sortedItems[0].slice(-1)[0]
                                    .legislation_id;
                            }
                        }
                        if (
                            ...
                            if (
                                ...
                                if (
                                    citationsInNote[citations[j - 1].properties.noteIndex] === 1 ||
                        ...
                        if (suprame || ibidme) {
                            if (first_ref[myid] != onecitation.properties.noteIndex) {
                                ...
                                if (this.registry.registry[myid]) {
                                    var oldFirst = this.registry.citationreg.citationsByItemId[
                                        myid
...
CSL.getAmbiguousCite = function(Item, disambig, visualForm, item) {
    ...
    if (
        ...
        if (this.citation.opt['givenname-disambiguation-rule'] === 'by-cite') {
            itemSupp['first-reference-note-number'] = this.registry.registry[Item.id][
    ...
    this.parallel.use_parallels =
        this.parallel.use_parallels === true || this.parallel.use_parallels === null ? null : false;
    ...
    for (pos = 0; pos < len; pos += 1) {
        ...
        if (pos > 0) {
            ...
            var precedingEndsInPeriodOrComma =
                preceding_item.suffix && ['.', ','].indexOf(preceding_item.suffix.slice(-1)) > -1;
    ...
    for (pos = 0, len = myblobs.length; pos < len; pos += 1) {
        ...
        if ('object' === typeof composite && composite.length === 0 && !item['suppress-author']) {
            if (this.tmp.has_purged_parallel) {
                ...
                var sufStr =
                    pos === myblobs.length - 1 ? txt_esc(this.citation.opt.layout_suffix) : '';
        ...
        if (buffer.length) {
            if ('string' === typeof buffer[0]) {
                if (pos > 0) {
                    if (
                        myblobs.length - 1 > pos &&
                        myparams[pos + 1].have_collapsed &&
                        !myparams[pos].have_collapsed
                    ) {
        ...
        if (!this.registry.registry[Item.id]) {
            ...
            if (
                ...
                if (this.opt['disambiguate-add-names']) {
                    this.tmp.disambig_settings.names = this.registry.registry[
                        Item.id
                    ].disambig.names.slice();
                    if (this.tmp.disambig_request) {
                        this.tmp.disambig_request.names = this.registry.registry[
                            Item.id
                        ].disambig.names.slice();
                    }
                }
                if (this.opt['disambiguate-add-givenname']) {
                    ...
                    this.tmp.disambig_settings.givens = this.registry.registry[
                        Item.id
                    ].disambig.givens.slice();
                    this.tmp.disambig_request.givens = this.registry.registry[
                        Item.id
                    ].disambig.givens.slice();
                    for (
                        ...
                        this.tmp.disambig_settings.givens[i] = this.registry.registry[
                            Item.id
                        ].disambig.givens[i].slice();
                    }
                    for (
                        ...
                        this.tmp.disambig_request.givens[i] = this.registry.registry[
                            Item.id
                        ].disambig.givens[i].slice();
                    }
                }
...
CSL.citeEnd = function(Item, item) {
    if (this.tmp.disambig_restore && this.registry.registry[Item.id]) {
        ...
        for (
            ...
            this.registry.registry[Item.id].disambig.givens[i] = this.tmp.disambig_restore.givens[
                i
            ].slice();
        }
    }
            ...
            if (this.locale[lang_out].terms[termname][genderform]) {
                for (form in this.locale[lang_out].terms[termname]) {
                    if (!this.locale[lang_out].terms[termname][genderform][form]) {
                        this.locale[lang_out].terms[termname][genderform][form] = this.locale[
                            lang_out
                        ].terms[termname][form];
                    }
                }
        ...
        if (this.tokentype === CSL.END) {
            state.opt.grouped_sort =
                (state.opt.xclass === 'in-text' &&
                    (state.citation.opt.collapse && state.citation.opt.collapse.length)) ||
    ...
    build: function(state, target) {
        ...
        if (this.tokentype === CSL.START) {
            ...
            if (this.locale_raw) {
                ...
                state.tmp.cite_affixes[state.build.area][
                    my_tok.locale
                ].delimiter = this.strings.delimiter;
                state.tmp.cite_affixes[state.build.area][
                    my_tok.locale
                ].suffix = this.strings.suffix;
            }
        }
        if (this.tokentype === CSL.END) {
            if (this.locale_raw) {
                ...
                if (state.build.area === 'citation') {
                    ...
                    func = function(state, Item, item) {
                        ...
                        if (item && item.suffix) {
                            sp = '';
                            if (
                                item.suffix.match(CSL.STARTSWITH_ROMANESQUE_REGEXP) ||
        ...
        if (!this.state.opt.development_extensions.spoof_institutional_affiliations) {
            ...
            for (var j = 0, jlen = this.institutions[v].length; j < jlen; j += 1) {
                institution_sets.push(
                    this.joinPersonsAndInstitutions([this.persons[v][j], this.institutions[v][j]])
                );
            }
...
CSL.NameOutput.prototype._normalizeVariableValue = function(Item, variable) {
    ...
    if ('string' === typeof Item[variable] || 'number' === typeof Item[variable]) {
        CSL.debug(
            'name variable "' + variable + '" is string or number, not array. Attempting to fix.'
        );
...
CSL.NameOutput.prototype._joinAnd = function(blobs, tokenname) {
    return this._join(
        blobs,
        this.state.inheritOpt(this[tokenname], 'delimiter', tokenname + '-delimiter', ', '),
...
CSL.NameOutput.prototype._renderInstitutionName = function(v, name, slot, j) {
    ...
    switch (this.institution.strings['institution-parts']) {
        ...
        default:
            long_style = this._getLongStyle(primary, v, j);
            institution = [
                this._composeOneInstitutionPart([n.l.pri, n.l.sec, n.l.ter], slot, long_style, v),
            ];
            break;
    }
...
CSL.NameOutput.prototype._composeOneInstitutionPart = function(names, slot, style, v) {
    ...
    if (secondary || tertiary) {
        ...
        if (slot.secondary) {
            secondary_tok.strings.prefix = this.state.opt.citeAffixes.institutions[
                slot.secondary
            ].prefix;
            secondary_tok.strings.suffix = this.state.opt.citeAffixes.institutions[
                slot.secondary
            ].suffix;
        ...
        if (slot.tertiary) {
            tertiary_tok.strings.prefix = this.state.opt.citeAffixes.institutions[
                slot.tertiary
            ].prefix;
            tertiary_tok.strings.suffix = this.state.opt.citeAffixes.institutions[
                slot.tertiary
            ].suffix;
...
CSL.NameOutput.prototype._renderPersonalName = function(v, name, slot, pos, i, j) {
    ...
    if (secondary || tertiary) {
        ...
        if (slot.secondary) {
            secondary_tok.strings.prefix = this.state.opt.citeAffixes.persons[
                slot.secondary
            ].prefix;
            secondary_tok.strings.suffix = this.state.opt.citeAffixes.persons[
                slot.secondary
            ].suffix;
            if (!secondary_tok.strings.prefix) {
                secondary_tok.strings.prefix = ' ';
...
CSL.NameOutput.prototype._renderOnePersonalName = function(value, pos, i, j) {
    ...
    if (romanesque === 0) {
        ...
        if (
            ['always', 'display-and-sort'].indexOf(this.state.opt['demote-non-dropping-particle']) >
            -1
        ) {
...
CSL.NameOutput.prototype._givenName = function(name, pos, i) {
    ...
    if (this.state.inheritOpt(this.name, 'initialize') === false) {
        ...
        if (
            name.family &&
            1 === this.state.tmp.disambig_settings.givens[pos][i] &&
...
CSL.NameOutput.prototype._parseName = function(name) {
    ...
    var noparse;
    if (
        (name.family && (name.family.slice(0, 1) === '"' && name.family.slice(-1) === '"')) ||
    ...
    if (this.state.sys.getAbbreviation) {
        ...
        for (var j = 0, jlen = long_form.length; j < jlen; j += 1) {
            ...
            if (this.state.transform.abbrevs[jurisdiction]['institution-part'][normalizedKey]) {
                short_form[j] = this.state.transform.abbrevs[jurisdiction]['institution-part'][
                    normalizedKey
                ];
                use_short_form = true;
            }
    ...
    if (this.institution.strings.form === 'short' && this.state.sys.getAbbreviation) {
        ...
        for (var j = splitInstitution.length; j > 0; j += -1) {
            ...
            if (this.state.transform.abbrevs[jurisdiction]['institution-entire'][normalizedKey]) {
                var splitLst = this.state.transform.abbrevs[jurisdiction]['institution-entire'][
                    normalizedKey
                ];
        ...
        if ([CSL.SINGLETON, CSL.START].indexOf(this.tokentype) > -1) {
            ...
            func = function(state, Item) {
                ...
                if (state.inheritOpt(this, 'delimiter-precedes-last') === 'always') {
                    ...
                    state.inheritOpt(this, 'delimiter-precedes-last') === 'after-inverted-name'
                ) {
    ...
    build: function(state, target) {
        ...
        if (this.tokentype === CSL.END) {
            ...
            func = function(state, Item, item) {
                ...
                if (
                    state.inheritOpt(state.nameOutput.name, 'delimiter-precedes-last') === 'always'
                ) {
                    ...
                    func = function(state, Item, item) {
                        ...
                        if (!state.tmp.just_looking) {
                            if (item && item['author-only']) {
                                ...
                                term = CSL.Output.Formatters['capitalize-first'](
                                    state,
                                    reference_term
                                );
                ...
                if (this.strings.term) {
                    func = function(state, Item, item) {
                        ...
                        if (
                            ...
                            !(state.opt['class'] === 'in-text' && state.tmp.area === 'citation')
                        ) {
    ...
    if ('label' === this.name && this.variables[0]) {
        ...
        func = function(state, Item, item) {
            ...
            for (var i = 0, ilen = this.variables_real.length; i < ilen; i++) {
                if (
                    state.tmp.done_vars.indexOf(this.variables_real[i]) === -1 &&
                    !(
                        item &&
                        Item.type === 'legal_case' &&
                        item['suppress-author'] &&
                        this.variables_real[i] === 'title'
                    )
                ) {
                ...
                if (
                    ...
                    if (Item.multi && Item.multi._keys[variable]) {
                        for (var langTag in Item.multi._keys[variable]) {
                            rawMultiNames[langTag] = Item.multi._keys[variable][langTag].split(
                                /\s*;\s*/
                            );
        ...
        if (nameList && nameList.length) {
            ...
            for (var i = 0, ilen = nameList.length; i < ilen; i += 1) {
                if (
                    ...
                    (nameList[i].literal ||
                        (nameList[i].isInstitution && nameList[i].family && !nameList[i].given))
                ) {
                    institutions += 1;
            ...
            if (this.sets.value().length > 0) {
                ...
                if (this.target === 'front') {
                    if (
                        ...
                        (!prev[this.variable] || this.data.value !== prev[this.variable].value)
                    ) {
                        ...
                        if (cite.prevItemID) {
                            if (!this.state.registry.registry[cite.prevItemID].parallel) {
                                ...
                                this.state.registry.registry[
                                    cite.itemId
                                ].parallel = this.state.registry.registry[cite.prevItemID].parallel;
                            }
                            this.state.registry.registry[
                                cite.itemId
                            ].siblings = this.state.registry.registry[cite.prevItemID].siblings;
...
CSL.Transform = function(state) {
        ...
        if (state.sys.getAbbreviation) {
            ...
            if (
                ...
                if (state.transform.abbrevs[jurisdiction][myabbrev_family][normalizedKey]) {
                    value = state.transform.abbrevs[jurisdiction][myabbrev_family][
                        normalizedKey
                    ].replace('{stet}', basevalue);
                }
            }
        }
        if (
            !value &&
            (!state.opt.development_extensions.require_explicit_legal_case_title_short ||
                Item.type !== 'legal_case') &&
            altvar &&
            Item[altvar] &&
            use_field
    ...
    function getTextSubField(Item, field, locale_type, use_default, stopOrig) {
        ...
        if (state.sys.getHumanForm && ret.name && field === 'jurisdiction') {
            ret.name = CSL.getJurisdictionNameAndSuppress(
                state,
                Item[field],
                jurisdictionName,
                this.strings.jurisdiction_depth
            );
    ...
    for (i = 0, ilen = namelist.length; i < ilen; i += 2) {
        if (isAbbrev[i]) {
            if (i < namelist.length - 2) {
                namelist[i + 1] = '';
                if (
                    (!terminator || (terminator.slice(-1) && terminator.slice(-1) !== ' ')) &&
                    namelist[i].length &&
                    namelist[i].match(CSL.ALL_ROMANESQUE_REGEXP) &&
...
CSL.Util.Names.doInitialize = function(state, namelist, terminator, mode) {
    ...
    for (i = 0, ilen = namelist.length; i < ilen; i += 2) {
        ...
        if (
            !m &&
            (!n.match(CSL.STARTSWITH_ROMANESQUE_REGEXP) && n.length > 1 && terminator.match('%s'))
        ) {
            m = n.match(/(.)(.*)/);
        }
        ...
        for (var i = 0, ilen = trygenders.length; i < ilen; i += 1) {
            ...
            if (ordinfo['whole-number'][str] && ordinfo['whole-number'][str][trygender]) {
                suffix = this.state.getTerm(
                    this.state.locale[this.state.opt.lang].ord['1.0.1']['whole-number'][str][
                        trygender
                    ],
                    false,
                    0,
...
CSL.Engine.prototype.processNumber = function(node, ItemObject, variable, type) {
            ...
            if (m) {
                ...
                if (m.length > 0 && i === 0) {
                    ...
                    if (
                        ...
                        (['locator', 'number'].indexOf(variable) === -1 &&
    ...
    function fixNumericAndCount(values, i, currentLabelInfo) {
        ...
        if (
            !isCollapsible &&
            i > 0 &&
            val.match(/^[ivxlcmIVXLCM]+$/) &&
            values[i - 1].value.match(/^[ivxlcmIVXLCM]+$/)
        ) {
            isCollapsible = true;
        }
        for (var j = currentLabelInfo.pos, jlen = values.length; j < jlen; j++) {
            if (
                currentLabelInfo.label === values[j].label &&
                ...
                if (values[i].label === currentLabelInfo.label) {
                    ...
                    if (currentLabelInfo.pos === 0) {
                        if (variable === 'locator' || variable === 'number') {
                            if (
                                !me.getTerm(CSL.STATUTE_SUBDIV_STRINGS[currentLabelInfo.label]) &&
                                currentLabelInfo.label.slice(0, 4) !== 'var:'
                            ) {
                        ...
                        if (['locator', 'number'].indexOf(variable) === -1) {
                            if (
                                CSL.STATUTE_SUBDIV_STRINGS[currentLabelInfo.label] !== variable &&
                                currentLabelInfo.label.slice(0, 4) !== 'var:'
                            ) {
                        ...
                        if (
                            values[i - 1].label !== values[i].label &&
                            currentLabelInfo.label.slice(0, 4) !== 'var:'
                        ) {
                            values[currentLabelInfo.pos].labelVisibility = true;
                        }
        ...
        if (!me.tmp.just_looking) {
            for (var j = masterNode.decorations.length - 1; j > -1; j--) {
                if (masterNode.decorations[j][0] === '@quotes') {
                    masterStyling.decorations = masterStyling.decorations.concat(
                        masterNode.decorations.slice(j, j + 1)
                    );
                    masterNode.decorations = masterNode.decorations
                        .slice(0, j)
                        .concat(masterNode.decorations.slice(j + 1));
                }
            }
    ...
    function fixupRangeDelimiter(variable, val, rangeDelimiter, isNumeric) {
        ...
        if (hasTerm && rangeDelimiter === '-') {
            if (isNumeric) {
                if (
                    isPage ||
                    ['locator', 'issue', 'volume', 'edition', 'number'].indexOf(variable) > -1
                ) {
    ...
    function checkPage(variable, val) {
        return variable === 'page' || (variable === 'locator' && ['p.'].indexOf(val.label) > -1);
    }
...
CSL.Disambiguation.prototype.getCiteData = function(Item, base) {
    if (!this.maxNamesByItemId[Item.id]) {
        ...
        this.state.registry.registry[
            Item.id
        ].disambig.givens = this.state.tmp.disambig_settings.givens.slice();
        for (
            var i = 0, ilen = this.state.registry.registry[Item.id].disambig.givens.length;
            i < ilen;
            i += 1
        ) {
...
CSL.Disambiguation.prototype.captureStepToBase = function() {
    if (
        ...
        if ('undefined' !== typeof this.base.givens[this.gnameset][this.gname]) {
            this.betterbase.givens[this.gnameset][this.gname] = this.base.givens[this.gnameset][
                this.gname
            ];
        }
    }
