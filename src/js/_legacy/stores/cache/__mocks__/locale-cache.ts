import { localeMapper } from '_legacy/utils/constants';
...
export default class LocaleCache {
    ...
    constructor() {
        const cache = localStorage.getItem(LocaleCache.CACHE_KEY);
        if (cache) {
            const localeJson: Cache = JSON.parse(cache);
